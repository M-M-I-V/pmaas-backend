...
public class InventoryImportService {
    ...
    public ImportResponse importFromExcel(MultipartFile file) {
        ...
        int totalRows = 0, inserts = 0, duplicates = 0, failed = 0;
        ...
        try (XSSFWorkbook workbook = new XSSFWorkbook(file.getInputStream())) {
            ...
            for (int si = 0; si < workbook.getNumberOfSheets(); si++) {
                ...
                Optional<ItemCategory> categoryOpt = ItemCategory.fromSheetName(sheetName);
                ...
                for (int ri = 1; ri <= sheet.getLastRowNum(); ri++) {
                    ...
                    if (itemName.isBlank()) {
                        rowErrors.add(new ImportRowError(sheetName, ri + 1, "itemName",
                                itemName, "Item name is required"));
                    }

                    Integer stockOnHand = parseInteger(stockStr);
                    if (stockOnHand == null) {
                        rowErrors.add(new ImportRowError(sheetName, ri + 1, "stockOnHand",
                                stockStr, "Must be a valid non-negative integer"));
                    } else if (stockOnHand < 0) {
                        rowErrors.add(new ImportRowError(sheetName, ri + 1, "stockOnHand",
                                stockStr, "Stock on hand cannot be negative"));
                        stockOnHand = null;
                    }

                    LocalDate expirationDate = parseDate(row, 4, formatter);
                    if (!expStr.isBlank() && expirationDate == null) {
                        rowErrors.add(new ImportRowError(sheetName, ri + 1, "expirationDate",
                                expStr, "Invalid date format — expected YYYY-MM-DD or Excel date"));
                    }

                    LocalDate dateReceived = parseDate(row, 5, formatter);
                    if (!recvStr.isBlank() && dateReceived == null) {
                        rowErrors.add(new ImportRowError(sheetName, ri + 1, "dateReceived",
                                recvStr, "Invalid date format — expected YYYY-MM-DD or Excel date"));
                    }
                    ...
                    if (repository.existsByItemNameIgnoreCaseAndDateReceivedAndCategory(
                            itemName, dateReceived, category)) {
                        duplicates++;
                        continue;
                    }
                    ...
                    InventoryItem item = InventoryItem.builder()
                            ...
                            .brandName(brandName.isBlank() ? null : brandName.trim())
                            .category(category)
            ...
            log.error("Failed to read Excel file '{}': {}", file.getOriginalFilename(), e.getMessage());
        ...
        auditLogService.record(
                "Inventory", 0, "IMPORT",
                String.format("File: %s | Rows: %d | Inserted: %d | Duplicates: %d | Failed: %d",
                        file.getOriginalFilename(), totalRows, inserts, duplicates, failed)
        );

        log.info("[IMPORT] user='{}' file='{}' rows={} inserts={} dupes={} failed={}",
                importedBy, file.getOriginalFilename(), totalRows, inserts, duplicates, failed);
